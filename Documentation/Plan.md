...
1. Техническая сложность настройки окружения. Для работы приложения нужно запуск двух баз данных (MySQL, PostgreSQL), развернуть симулятор платежной системы, написанный на другом языке. Возможны проблемы при настройки CI (если потребуется). Поэтому в ходе настройки окружения могут возникнуть сложности, требующие дополнительных временных затрат.
...
## План сдачи работ:
После утверждения плана дипломным руководителем на написание авто-тестов 5 дней рабочих, на подготовку отчетов 3 раб дня.